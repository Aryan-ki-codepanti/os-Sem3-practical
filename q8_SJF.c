...
int get_next_shortest(int time_counter, int arrival[], int burst[], int n)
{
    int shortest_process = process_with_non_zero_burst(burst, n);
    for (int process = 0; process < n; process++)
    {
        if (burst[process] != 0 && arrival[process] <= time_counter && burst[process] < burst[shortest_process])
...
int main()
{
    ...
    {
        current_process = get_next_shortest(time_counter, arrival, burst, n);
        current_time_used = burst[current_process];
        time_counter += current_time_used;
        burst[current_process] = 0; // task completed
